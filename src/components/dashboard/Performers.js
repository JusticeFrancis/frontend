...
import { Button, Divider, Drawer, IconButton, Link } from "@mui/material";
import FilterAltIcon from "@mui/icons-material/FilterAlt";
import SearchIcon from "@mui/icons-material/Search";
import { TuneOutlined } from "@mui/icons-material";
import Notice from "../../elements/Notice";
import FilterMenu from "../../elements/FilterMenu";
import Performer from "../../elements/Performer";

const Performers = () => {
  const [showFilter, setShowFilter] = useState(false);
  const [openFilterModal, setOpenFilterModal] = useState(false)
  return (
    <div className="lg:p-6 lg:px-[100px] grid lg:grid-cols-8">
      ...
      <div className={showFilter ? "col-span-2 " : "col-span-2 hidden"}>
       ...
       <FilterMenu setShowFilter={setShowFilter} setOpenFilterModal={setOpenFilterModal}  showFilter={showFilter} />
       </div>
      </div>
      <div
        className={
          showFilter ? "bg-white  p-3 col-span-6" : "bg-white  p-3 col-span-8 "
        }
      >
        <h1 className="lg:text-4xl text-2xl  font-bold mb-6">
          Find performer for your project
        </h1>

        <div className="lg:flex lg:space-x-4 lg:space-y-0 space-y-4 items-center mb-6 text-[14px] lg:text-[17px]">
          <div
            className="border-[1px] border-gray-300 p-2 rounded-[20px] px-4 cursor-pointer"
            onClick={() => {
              setShowFilter(true);
        ...
        <Divider className="" sx={{ mb: 2 }} />

        <div className={showFilter ? "grid lg:grid-cols-3  gap-4" : "grid lg:grid-cols-4  gap-4"}  >
            <Performer/>
            <Performer/>
            <Performer/>
            <Performer/>
